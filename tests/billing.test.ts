...
async function main() {
  ...
  const { getUserEntitlements, effectiveTier } = await import("../src/lib/services/entitlements");
  ...
  const priceCatalogue = new Map<
    string,
    { currency: string; unit_amount: number; interval: string }
  >([
  ...
  const spy: StripeClient = {
    ...
    async updateSubscriptionCancellation(p) {
      ...
      s.cancel_at = p.cancelAtPeriodEnd ? (s.items?.data?.[0]?.current_period_end ?? null) : null;
      return s;
    },
  ...
  const subRow = (userId: string) => db.subscription.findUniqueOrThrow({ where: { userId } });
  ...
  try {
    ...
    let aliceSession = "";
    await check(
      "case 1a: checkout session for PLUS carries the PLUS price id and metadata",
      async () => {
    ...
    await check(
      "case 2: invalid plan / extra keys are rejected by the strict schema (422 path)",
      () => {
        assert.equal(checkoutSchema.safeParse({ plan: "DIAMOND" }).success, false);
        assert.equal(
          checkoutSchema.safeParse({ plan: "PLUS", priceId: "price_evil" }).success,
          false,
        );
    ...
    await check(
      "case 4: double-tap inside the idempotency window returns the SAME session",
      async () => {
        const before = checkoutSessionsCreated;
        const carol = await seedUser("carol");
        const [r1, r2] = [await startCheckout(carol, "PLUS"), await startCheckout(carol, "PLUS")];
    ...
    await check(
      "case 5: creating a checkout session (the success_url flow) grants NOTHING",
      async () => {
    ...
    await check(
      "case 7: checkout-status answers PENDING/plan FREE while the session is open",
      async () => {
    ...
    await check(
      "duplicate checkout while subscribed answers already_subscribed (409 path)",
      async () => {
        await assert.rejects(
    ...
    await check(
      "case 13/webhook: checkout.session.completed syncs GOLD ACTIVE + payment recorded",
      async () => {
        const event = {
          id: nextEventId(),
          type: "checkout.session.completed",
    ...
    await check(
      "case 11: a duplicate delivery of the SAME event id is acknowledged without re-processing",
      async () => {
    ...
    await check(
      "ledger crash-retry: an unprocessed event row is reprocessed, not skipped",
      async () => {
        const eventId = nextEventId();
        await db.stripeEvent.create({
          data: { id: eventId, type: "customer.subscription.updated" },
        });
    ...
    await check(
      "case 14: subscription.updated remaps Gold -> Plus via the trusted price map",
      async () => {
    ...
    await check(
      "case 12: an UNKNOWN price id never grants a paid tier (audit-logged)",
      async () => {
        subscriptions.set(bobSub.id, {
          ...subscriptions.get(bobSub.id)!,
        ...
        await syncStripeSubscription({
          stripeCustomerId: bobCustomer,
          stripeSubscriptionId: bobSub.id,
        });
        const row = await subRow(bob);
        assert.equal(row.tier, "FREE");
        assert.equal(row.status, "ACTIVE");
    ...
    await check(
      "case 15: cancelAtPeriodEnd keeps entitlements until the period actually ends",
      async () => {
        subscriptions.set(bobSub.id, {
          ...subscriptions.get(bobSub.id)!,
    ...
    await check(
      "case 17: invoice.payment_failed -> PAST_DUE with a bounded dunning grace",
      async () => {
        const nowSecs = Math.floor(Date.now() / 1000);
        subscriptions.set(bobSub.id, {
          ...subscriptions.get(bobSub.id)!,
        ...
        const event = {
          ...
          data: {
            object: { id: `in_test_${RUN}`, customer: bobCustomer, subscription: bobSub.id },
          },
        };
        assert.equal((await webhookPOST(signedWebhookRequest(event))).status, 200);
        const row = await subRow(bob);
    ...
    await check(
      "case 16: subscription.deleted -> FREE (refetch-latest sees canceled)",
      async () => {
        subscriptions.set(bobSub.id, {
          ...subscriptions.get(bobSub.id)!,
          status: "canceled",
    ...
    await check(
      "checkout.session.expired clears CHECKOUT_PENDING back to implicit FREE",
      async () => {
    ...
    await check(
      "webhook events for unmapped customers change nothing (no metadata trust)",
      async () => {
    ...
    // -----------------------------------------------------------------------
    await check(
      "case 18: portal uses the STORED customer only; users without one are refused",
      async () => {
    ...
    await check("hierarchy: upgrade targets derive from canonical FREE < PLUS < GOLD", () => {
      ...
      assert.deepEqual(
        upgradePlansFor("FREE").map((p) => p.tier),
        ["PLUS", "GOLD"],
      );
      assert.deepEqual(
        upgradePlansFor("PLUS").map((p) => p.tier),
        ["GOLD"],
      );
      assert.deepEqual(
        upgradePlansFor("GOLD").map((p) => p.tier),
        [],
      );
    ...
    await check(
      "change-plan schema is as strict as checkout (no price ids from the browser)",
      () => {
    ...
    let aliceSubId = "";
    await check(
      "PLUS -> GOLD: paid upgrade updates the EXISTING subscription in place (no duplicate)",
      async () => {
        ...
        assert.equal(await db.payment.count({ where: { userId: alice } }), paymentsBefore + 1);
    ...
    await check(
      "the customer.subscription.updated webhook after an upgrade is a harmless no-op",
      async () => {
        const event = {
          id: nextEventId(),
          type: "customer.subscription.updated",
    ...
    await check(
      "GOLD has no upgrade: same tier and downgrades answer invalid_plan_change",
      async () => {
        await assert.rejects(
    ...
    await check(
      "preview returns the EXACT Stripe proration - nothing charged, nothing changed",
      async () => {
    ...
    await check(
      "requires_action: plan UNCHANGED, clientSecret returned to the owner only",
      async () => {
        upgradeOutcome = "requires_action";
    ...
    await check(
      "upgrade lock: a second request while one awaits payment answers upgrade_pending",
      async () => {
        await assert.rejects(
    ...
    await check(
      "pending_update_applied (payment succeeded) activates Gold + records the invoice",
      async () => {
    ...
    await check(
      "combined lifecycle: cancel after a PAID upgrade shows ENDING, resume restores",
      async () => {
    ...
    await check(
      "declined card: PAYMENT_FAILED, Plus unchanged, failed upgrade invoice in history",
      async () => {
        upgradeOutcome = "declined";
    ...
    await check(
      "double tap: concurrent identical upgrades produce ONE Stripe application",
      async () => {
        upgradeOutcome = "paid";
    ...
    await check(
      "zero-due upgrade applies ONLY on Stripe's confirmation (fresh price, no pending)",
      async () => {
    ...
    await check(
      "portal cancel: cancel_at_period_end flows back via webhook reconciliation",
      async () => {
        await syncStripeSubscription({
    ...
    await check(
      "resume clears cancel_at_period_end on the SAME subscription - nothing new is created",
      async () => {
    ...
    await check(
      "resume guards: not scheduled -> not_ending; no subscription -> no_subscription",
      async () => {
        await assert.rejects(
    ...
    await check(
      "reconcile-on-view: a portal cancellation is visible with NO webhook at all",
      async () => {
        const s = subscriptions.get(harrySub.id)!;
    ...
    await check(
      "expired: subscription.deleted keeps the PRIOR plan story, grants nothing",
      async () => {
        const s = subscriptions.get(harrySub.id)!;
        s.status = "canceled";
    ...
    await check("invoice.payment_failed records a FAILED payment row + PAST_DUE", async () => {
      await syncStripeSubscription({
        stripeCustomerId: irisCustomer,
        stripeSubscriptionId: irisSub.id,
      });
    ...
    await check(
      "retry payment: declined card answers payment_failed and changes nothing",
      async () => {
    ...
    await check(
      "retry payment: success collects the invoice and reactivates via the shared sync",
      async () => {
    ...
    await check(
      "case 19d: deep validation verifies EUR/monthly/amounts via the client",
      async () => {
        resetStripeEnvDeepCache();
    ...
    if (!reachable) {
      skip(
        "case 3: unauthenticated POST /api/billing/checkout answers 401",
        "dev server not running",
      );
      skip(
        "unauthenticated GET /api/billing/checkout-status answers 401",
        "dev server not running",
      );
      ...
      skip(
        "unauthenticated POST /api/billing/resume and /retry-payment answer 401",
        "dev server not running",
      );
    } else {
