...
async function main() {
  ...
  try {
    ...
    await check("subscribe upserts by endpoint (register twice -> one row, keys refreshed)", async () => {
      ...
      const rows = await db.pushSubscription.findMany({ where: { endpoint: aliceEndpoint } });
      assert.equal(rows.length, 1);
      assert.equal(rows[0].p256dh, "p256dh-v2");
      assert.equal(rows[0].userId, alice);
    });

    await check("an endpoint owned by another user is rebound to the caller + audited", async () => {
      ...
      const row = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: aliceEndpoint } });
    ...
    await check("unsubscribe revokes own subscription only, keeps the row", async () => {
      ...
      const again = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: aliceEndpoint } });
      assert.equal(again.enabled, true);
      assert.equal(again.revokedAt, null);
    });
    ...
    await check("status lists active devices with truncated endpoints + vapid key", async () => {
      ...
      assert.ok(devices[0].endpoint.length <= 60);
    ...
    await check("disabled / empty / null windows are never quiet", () => {
      ...
      assert.equal(
        evaluateQuietHours(
          { quietHoursEnabled: false, quietHoursStart: 0, quietHoursEnd: 1439, timezone: "Etc/UTC" },
          noon,
        ),
        false,
      );
    ...
    await check("EMAIL/SMS rows exist only for safety/account types, honestly stubbed", async () => {
    ...
    await check("multi-device fanout: one 410 revokes that device, the other succeeds -> SENT", async () => {
      ...
      const dead = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: aliceEndpoint } });
      assert.equal(dead.enabled, false);
      assert.ok(dead.revokedAt);
      const alive = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: aliceEndpoint2 } });
    ...
    await check("5 accumulated failures permanently disable a subscription", async () => {
      ...
      const row = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: aliceEndpoint2 } });
      assert.equal(row.enabled, false);
      assert.equal(row.failureCount, 5);
      failWith.delete(aliceEndpoint2);
    });

    await check("dispatch with zero active devices goes DEAD (no_active_subscriptions)", async () => {
    ...
    await check("sendMessage notifies the recipient, never the sender, with private push copy", async () => {
      ...
      const bobNotif = await db.notification.findFirst({
        where: { userId: bob, type: "NEW_MESSAGE", data: { path: ["messageId"], equals: message.id } },
      });
      assert.ok(bobNotif, "recipient got an in-app notification");
      const aliceNotif = await db.notification.findFirst({
        where: { userId: alice, type: "NEW_MESSAGE", data: { path: ["messageId"], equals: message.id } },
      });
      ...
      const payload = JSON.parse(sent[0].payload);
      assert.ok(!sent[0].payload.includes("launch codes"), "push payload never carries chat text");
    ...
    await check("stale subscriptions get revoked by the cron sweep", async () => {
      ...
      assert.ok(revoked >= 1);
      const row = await db.pushSubscription.findUniqueOrThrow({ where: { endpoint: staleEndpoint } });
      assert.equal(row.enabled, false);
